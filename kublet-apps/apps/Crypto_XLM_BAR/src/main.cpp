...
float lastXlmPrice = NAN;
float lastBarPrice = NAN;
...
#define COLOR_XLM   0x07FF   // stellar cyan-blue
#define COLOR_BAR   0xFFE0   // bar yellow
...
//   {"symbol":"XLMUSDT","price":"43210.50000000"}
...
float fetchXlmPrice() { return fetchBinancePrice("XLMUSDT"); }
float fetchBarPrice() { return fetchBinancePrice("BARUSDT"); }
...
void drawHeader() {
  ...
  const char *title = "XLM / BAR  -  CRYPTO";
...
void drawCoinRow(int y0, const char *label, uint16_t labelColor, float price, int direction) {
  ...
  // XLM trades well under $1 currently; BAR is a smaller-cap symbol whose
  // price we don't pin down here. Use the same auto-format as the BTC/ETH
  // app: whole numbers for >=$1000, two decimals otherwise. Sub-$1 coins
  // still render as "$0.xx" which is correct for XLM.
  char buf[20];
...
void drawUI(float xlm, float bar, time_t lastFetch, bool fetchOk) {
  ui.tft.fillScreen(TFT_BLACK);
  drawHeader();
  drawCoinRow(48,  "XLM-USD", COLOR_XLM, xlm, xlmDirection);
  ui.tft.drawFastHLine(0, 130, 240, COLOR_DGRAY);
  drawCoinRow(132, "BAR-USD", COLOR_BAR, bar, barDirection);
  drawFooter(fetchOk, lastFetch);
}

void setup() {
  Serial.begin(460800);
  Serial.println("\n=== crypto XLM/BAR app starting ===");
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (shouldFetch) {
      float xlm = fetchXlmPrice();
      float bar = fetchBarPrice();
      bool ok = !isnan(xlm) && !isnan(bar);
      ...
      if (!isnan(xlm)) {
        if (!isnan(lastXlmPrice)) {
          if      (xlm > lastXlmPrice + 0.005f) xlmDirection = +1;
          else if (xlm < lastXlmPrice - 0.005f) xlmDirection = -1;
          else                                  xlmDirection = 0;
        }
        lastXlmPrice = xlm;
      }
      if (!isnan(bar)) {
        if (!isnan(lastBarPrice)) {
          if      (bar > lastBarPrice + 0.005f) barDirection = +1;
          else if (bar < lastBarPrice - 0.005f) barDirection = -1;
          else                                  barDirection = 0;
        }
        lastBarPrice = bar;
      }
      ...
      drawUI(lastXlmPrice, lastBarPrice, lastFetchEpoch, lastFetchOk);
    }

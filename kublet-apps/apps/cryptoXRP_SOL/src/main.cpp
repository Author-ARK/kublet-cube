...
float lastXrpPrice = NAN;
float lastSolPrice = NAN;
...
#define COLOR_XRP   0xC7FC   // ripple pale-blue / white
#define COLOR_SOL   0xC81F   // solana magenta-purple
...
//   {"symbol":"XRPUSDT","price":"43210.50000000"}
...
float fetchXrpPrice() { return fetchBinancePrice("XRPUSDT"); }
float fetchSolPrice() { return fetchBinancePrice("SOLUSDT"); }
...
void drawHeader() {
  ...
  const char *title = "XRP / SOL  -  CRYPTO";
...
void drawCoinRow(int y0, const char *label, uint16_t labelColor, float price, int direction) {
  ...
  // Whole-number USD only (no .00). XRP trades in single dollars, SOL in
  // low hundreds — both well above $1 nowadays. If a coin ever drops below
  // $1, fall back to two decimals so the display isn't just "$0".
  char buf[20];
...
void drawUI(float xrp, float sol, time_t lastFetch, bool fetchOk) {
  ui.tft.fillScreen(TFT_BLACK);
  drawHeader();
  drawCoinRow(48,  "XRP-USD", COLOR_XRP, xrp, xrpDirection);
  ui.tft.drawFastHLine(0, 130, 240, COLOR_DGRAY);
  drawCoinRow(132, "SOL-USD", COLOR_SOL, sol, solDirection);
  drawFooter(fetchOk, lastFetch);
}

void setup() {
  Serial.begin(460800);
  Serial.println("\n=== crypto XRP/SOL app starting ===");
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (shouldFetch) {
      float xrp = fetchXrpPrice();
      float sol = fetchSolPrice();
      bool ok = !isnan(xrp) && !isnan(sol);
      ...
      if (!isnan(xrp)) {
        if (!isnan(lastXrpPrice)) {
          if      (xrp > lastXrpPrice + 0.005f) xrpDirection = +1;
          else if (xrp < lastXrpPrice - 0.005f) xrpDirection = -1;
          else                                  xrpDirection = 0;
        }
        lastXrpPrice = xrp;
      }
      if (!isnan(sol)) {
        if (!isnan(lastSolPrice)) {
          if      (sol > lastSolPrice + 0.005f) solDirection = +1;
          else if (sol < lastSolPrice - 0.005f) solDirection = -1;
          else                                  solDirection = 0;
        }
        lastSolPrice = sol;
      }
      ...
      drawUI(lastXrpPrice, lastSolPrice, lastFetchEpoch, lastFetchOk);
    }

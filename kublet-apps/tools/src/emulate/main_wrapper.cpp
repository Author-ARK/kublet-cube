...
// Continuous frame dump (for webui "Simulator" tab): --frame-dump PATH --frame-interval MS
// Overwrites a single file atomically (write-to-tmp, rename) so the webui can
// poll it without ever reading a half-written PNG.
static const char* g_frame_dump_path = nullptr;
static uint32_t g_frame_interval_ms = 250;
static uint32_t g_frame_last_dump_ms = 0;
...
// ---------------------------------------------------------------------------
// Continuous frame dump for webui live view
// ---------------------------------------------------------------------------
static void dumpLiveFrame(const char* path, const uint16_t* fb) {
  char tmp[1024];
  snprintf(tmp, sizeof(tmp), "%s.tmp", path);
  uint8_t* rgb = new uint8_t[240 * 240 * 3];
  fb565_to_rgb888(fb, rgb, 240, 240);
  if (stbi_write_png(tmp, 240, 240, 3, rgb, 240 * 3)) {
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--scale") == 0 && i+1 < argc) {
      ...
      g_frame_dump_path = argv[++i];
    } else if (strcmp(argv[i], "--frame-interval") == 0 && i+1 < argc) {
      g_frame_interval_ms = atoi(argv[++i]);
      if (g_frame_interval_ms < 50) g_frame_interval_ms = 50;  // 20 fps cap
  ...
  while (g_running) {
    ...
    // Continuous frame dump for webui live view (wall-time based)
    if (g_frame_dump_path && g_display &&
        elapsed - g_frame_last_dump_ms >= g_frame_interval_ms) {
      dumpLiveFrame(g_frame_dump_path, g_display->getFramebuffer());
      g_frame_last_dump_ms = elapsed;
    }

...
def write_user_setup(app_dir: Path) -> None:
    ...
    if not setup_path.parent.exists():
        pio = find_pio()
        print("  Dependencies not installed. Running pio pkg install...")
        # `pio lib install` was deprecated in PlatformIO 6.1 in favor of the
        # unified package manager command. Same behavior, no warning.
        subprocess.run([pio, "pkg", "install"], cwd=app_dir, check=True)
...
def ota_send(ip: str, firmware: Path, app_name: str) -> None:
    ...
    try:
        # 300s upload window. A 1 MB firmware on a marginal WiFi link can
        # legitimately take 3-5 minutes; the previous 60s cap killed slow
        # but progressing uploads (cubes far from the AP, 2.4 GHz channel
        # congestion, etc.). Server-side cube's OTAServer.handle() keeps
        # the connection alive for the duration of the multipart POST.
        conn = http.client.HTTPConnection(ip, timeout=300)

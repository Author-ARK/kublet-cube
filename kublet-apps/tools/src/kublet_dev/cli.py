...
def cmd_deploy(args: argparse.Namespace) -> None:
    """Build and send an app to the Kublet. WiFi OTA by default; --usb falls
    back to esptool flashing over serial (useful when WiFi signal is too weak
    for a 1 MB OTA push to complete in time)."""
    ...
    if args.usb:
        # USB deploy now mirrors OTA's partition handling: read otadata,
        # write the new firmware to the inactive ota slot, then bump
        # otadata to point at it. The running image is never overwritten
        # in-place, and OTA rotation continues naturally across USB and
        # WiFi deploys. See smart_usb_deploy() in kublet_dev/flash.py.
        from kublet_dev.flash import detect_serial_port, smart_usb_deploy
        port = args.port or detect_serial_port()
        smart_usb_deploy(port, firmware_bin, app_name)
    else:
...
examples:
  ...
  ./tools/dev logs                            # stream logs (auto-detect port)
    ...
    p_deploy.add_argument(
        "--usb",
        action="store_true",
        help="USB fallback: flash app firmware via esptool instead of OTA "
             "(useful when WiFi signal is too weak for the OTA push to finish)",
    )
    p_deploy.add_argument(
        "-p",
        "--port",
        help="Serial port for --usb (auto-detected if omitted)",
    )
